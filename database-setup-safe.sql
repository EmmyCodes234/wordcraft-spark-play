-- WordSmith Social Features Database Setup (Safe Version)
-- This version handles existing data gracefully and avoids conflicts
...
-- 8. Profiles Table (if not exists) - Add user_code column if it doesn't exist
...
-- Add user_code column to existing profiles table if it doesn't exist
DO $$
BEGIN
...
-- Drop existing policies for friend requests
...
-- Drop existing policies for community events
DROP POLICY IF EXISTS "Users can view all community events" ON community_events;
DROP POLICY IF EXISTS "Admins can create community events" ON community_events;
DROP POLICY IF EXISTS "Admins can update community events" ON community_events;
DROP POLICY IF EXISTS "Admins can delete community events" ON community_events;
...
-- Drop existing policies for event participants
...
-- Drop existing policies for user stats
...
-- Success message
SELECT 'Social features database setup completed successfully!' as message;
